...
namespace Pacmio
{
    ...
    public sealed class InteractiveBrokersAccount : Account
    {
        ...
        public InteractiveBrokersAccount(string accountCode)
        {
            AccountCode = accountCode;
        }

        public override void Entry(Contract c, double quantity)//, double limit = double.NaN, double stop = double.NaN)
        {
            ...
            if (!ps.PendingEntry)
            {
                ...
                ps.EntryOrder = new OrderInfo()
                {
                    ...
                    //LimitPrice = limit,
                    Quantity = qty,
                    Transmit = true, // double.IsNaN(ps.Stop), // (double.IsNaN(ps.Limit) && double.IsNaN(ps.Stop)),
                    OutsideRegularTradeHours = false,
                    Description = EntryOrderDescription
                };

                Root.IBClient.PlaceOrder(ps.EntryOrder);
            }
        }
        ...
        public void EntryBraket(Contract c, double quantity, double stopLoss, double profitLimit)
        {
            ...
            ps.EntryOrder = new OrderInfo()
            {
                ...
                //LimitPrice = limit,
                Quantity = qty,
                Transmit = double.IsNaN(ps.Limit) && double.IsNaN(ps.Stop),
                OutsideRegularTradeHours = false,
                Description = EntryOrderDescription
            };
            ...
            int parentId = ps.EntryOrder.OrderId;
            ...
            if (!double.IsNaN(stopLoss))
            {
                OrderInfo StopLossOrder = new OrderInfo()
                {
                    ...
                    AuxPrice = ps.Stop,
                    Quantity = -qty,
                    Transmit = double.IsNaN(ps.Limit),
                    OutsideRegularTradeHours = true,
                    ParentOrderId = parentId,
                    Description = "Stop Loss Order"
                };
                Root.IBClient.PlaceOrder(StopLossOrder);
            }
            ...
            if (!double.IsNaN(profitLimit))
            {
                OrderInfo ProfitTakingOrder = new OrderInfo()
                {
                    ...
                    OutsideRegularTradeHours = true,
                    ParentOrderId = parentId,
                    Description = "Profit Taker Order"
                };
                Root.IBClient.PlaceOrder(ProfitTakingOrder);
            }
        }


        /*
    public override void Modify(Contract c, double stopLoss, double profitLimit)
    {
        PositionStatus ps = GetPosition(c);
        ...
        if(ps.Quantity != 0) 
        {
            ...
            if (!double.IsNaN(stopLoss))
            {
                if (ps.StopLossOrder is null || !ps.StopLossOrder.IsEditable)
                {
                    ps.StopLossOrder = new OrderInfo()
                    {
                        Account = AccountCode,
                        Contract = c,
                        Type = OrderType.Stop,
                ...
                Root.IBClient.PlaceOrder(ps.StopLossOrder);
            }
            else if (ps.StopLossOrder.IsEditable)
            {
                Root.IBClient.CancelOrder(ps.StopLossOrder.OrderId);
                ps.StopLossOrder = null;
            }

            if (!double.IsNaN(profitLimit))
            {
                if (ps.ProfitTakingOrder is null || !ps.ProfitTakingOrder.IsEditable)
                {
                    ps.ProfitTakingOrder = new OrderInfo()
                    {
                        Account = AccountCode,
                        Contract = c,
                        Type = OrderType.Limit,
                ...
                {
                    ps.ProfitTakingOrder.LimitPrice = ps.Limit;
                    ps.ProfitTakingOrder.Quantity = -ps.Quantity;
                }

                Root.IBClient.PlaceOrder(ps.ProfitTakingOrder);
            }
            else if (ps.ProfitTakingOrder.IsEditable)
            {
                Root.IBClient.CancelOrder(ps.ProfitTakingOrder.OrderId);
                ps.ProfitTakingOrder = null;
            }
        }
    }*/
        ...
        public override void Exit(Contract c)
        {
            ...
            if (!ps.PendingExit)
            {
                ...
                foreach (var order in OrderManager.GetActiveOrder(this, c))
                {
                    Root.IBClient.CancelOrder(order.OrderId);
                }

                //ps.StopLossOrder = null;
                //ps.ProfitTakingOrder = null;
                /*
                if (ps.EntryOrder.IsEditable)
                {
                    Root.IBClient.CancelOrder(ps.StopLossOrder.OrderId);
                    ps.EntryOrder = null;
                }
                ...
                /*
                foreach (var item in ac.Positions)
                {
                    OrderInfo od = new OrderInfo()
                    {
                        ...
                        LimitPrice = 0,// TextBoxOrderLimitPrice.Text.ToDouble(0),
                        AuxPrice = 0, //TextBoxOrderStopPrice.Text.ToDouble(0),
                        TimeInForce = OrderTimeInForce.GoodUntilCanceled,
                        Account = ac.AccountCode,
                        OutsideRegularTradeHours = true,
                    };
                ...
                OrderInfo od = new OrderInfo()
                {
                    ...
                    Transmit = true,
                    //OutsideRegularTradeHours = true,
                    Description = ExitOrderDescription
                };
        ...
        [IgnoreDataMember]
        public double TotalSize => GrossPositionValue + BuyingPower;
        ...
        public override string AccountCode { get; protected set; }
        ...
        public override double BuyingPower { get; protected set; } = double.NaN; // "198153.11"
        ...
        [Description("")]
        public double GrossPositionValue { get; set; } = double.NaN; // "1945.42"

        [DataMember, Browsable(true), ReadOnly(true), Category("Market Value"), DisplayName("Gross Position Value S")]
        [Description("")]
        public double GrossPositionValue_S { get; private set; } = double.NaN; // "1945.42"
        ...
        public void UpdateTags(string tagName, string value)
        {
            switch (tagName)
            {
                case ("AccountCode"):
                ...
                case ("GrossPositionValue"):
                    GrossPositionValue = value.ToDouble();
                    break;
                case ("GrossPositionValue-S"):
                    GrossPositionValue_S = value.ToDouble();
                    break;
                case ("Guarantee"):
