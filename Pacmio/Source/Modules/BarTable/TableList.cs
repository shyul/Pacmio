...
namespace Pacmio
{
    ...
    public static class TableList
    {
        ...
        public static MultiPeriod<(double Price, double Volume)> BarTableAdjust(this StockData sd, bool includeDividend = false)
        {
            MultiPeriod<(double Price, double Volume)> list = new MultiPeriod<(double Price, double Volume)>();

            var split_list = sd.SplitTable.Select(n => (n.Key, true, n.Value.Split));
            var dividend_list = sd.DividendTable.Select(n => (n.Key, false, n.Value.Dividend / n.Value.Close));
