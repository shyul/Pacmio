...
namespace Pacmio
{
    public sealed class Gain : BarAnalysis
    {
        public Gain(NumericColumn column, int maximumPeakProminence)
        {
            ...
            string label = Column is null ? "(error)" : (Column == TableList.Column_Close ? "(" + MaximumPeakProminence.ToString() + ")" : "(" + Column.Name + "," + MaximumPeakProminence.ToString() + ")");
            ...
            Description = (Column == TableList.Column_Close) ? GetType().Name : GetType().Name + " (" + Column.Name + ")";
        }
