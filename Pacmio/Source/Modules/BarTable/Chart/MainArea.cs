...
namespace Pacmio
{
    public sealed class MainArea : Area
    {
        ...
        public MainArea(BarChart chart, string name, int height, float leftAxisRatio = 0.3f) : base(chart, name, height)
        {
            ...
            PriceSeries.TagColumns.Add(TableList.Column_PeakTags);

            // Configure volume series
            // ===================================================
            AddSeries(VolumeSeries = new AdColumnSeries(TableList.Column_Volume, TableList.Column_Percent, 50)
            {
                Order = int.MinValue,
                Side = AlignType.Left,
                Name = TableList.Column_Volume.Name,
                LegendName = "VOLUME",
                LegendLabelFormat = "0.##"
            });
